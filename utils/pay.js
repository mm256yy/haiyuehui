...
function giftPay(param){
  ...
  util.jhxLoadShow("支付中");
  return new Promise(function(resolve, reject){
    util.request(api.GiftPay ,param, 'POST').then(res => {
      console.log(res);
      if(res.result == null){  //余额支付
        app.globalData.canPay = true;
        //回调支付是否成功
        util.request(api.GiftCallback ,{orderId:orderIdCallback}, 'GET').then(function(res) {
          console.log(res);
        });
        util.jhxLoadHide();
        resolve(true);
        ...
        wx.requestPayment({
          ...
          success (res) {
            ...
            if(res.errMsg == "requestPayment:ok"){  //成功
              //回调支付是否成功
              util.request(api.GiftCallback ,{orderId:orderIdCallback,ssn:ssn}, 'POST').then(function(res) {
                console.log(res);
              });
              util.jhxLoadHide();
              resolve(true);
